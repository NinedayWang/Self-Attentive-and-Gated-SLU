...
class GatedAttentionSLU(nn.Module):
    ...
    def forward(self, input_w, input_l, seq_lens):
        ...
        expand_slot = [linear_slot[i][:seq_lens[i], :] for i in range(0, len(seq_lens))]
...
class Embedding(nn.Module):
    ...
    def __init__(self, word_vocab_num, letter_vocab_num,
        ...
        self._convolution_layer = nn.Conv1d(
            in_channels=letter_embedding_dim,
            out_channels=word_embedding_dim // 2,
    ...
    def forward(self, input_w, input_l):
        ...
        conv_l = self._convolution_layer(dropout_l.transpose(2, 1))
        pool_l = torch.mean(conv_l, dim=-1).view(l_size[0], l_size[1], -1)

        combine_vec = torch.cat([dropout_w, pool_l], dim=2)
        return combine_vec
